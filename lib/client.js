...
.gdb-btn.gdb-primary{background:var(--dsw-alias-state-business-primary);color:var(--dsw-alias-label-primary-inverted);border:none}
...
.gdb-load.gdb-confirm{color:var(--dsw-alias-label-primary-inverted);background:var(--dsw-alias-state-error-primary);border-color:transparent}
...
`
    ...
    function BackupsSection() {
      ...
      const [paths, setPaths] = React.useState({ coinCmd: '', coinCmdExists: false, desktopShortcut: '', desktopShortcutExists: false })

      const refresh = async () => {
        try {
          const r = await api('state')
          if (r && r.ok) { setProfiles(r.profiles || []); setKeep(r.keepSnapshots); setKeepInput(String(r.keepSnapshots)); setDesktopShortcut(r.desktopShortcut !== false); setPaths(r.paths || { coinCmd: '', coinCmdExists: false, desktopShortcut: '', desktopShortcutExists: false }); setHasCustomIcon(!!(r.hasCustomIcon)); setHasPythonPil(!!(r.hasPythonPil)) }
      ...
      // ── 自定义快捷方式图标：上传 / 清除 ──
      const [hasCustomIcon, setHasCustomIcon] = React.useState(false)
      const [hasPythonPil, setHasPythonPil] = React.useState(false)
      const fileInputRef = React.useRef(null)

      const doUploadIcon = async (file) => {
        if (!file) return
        setBusy(true); setStatus({ text: '', kind: '' })
        try {
          const buf = await file.arrayBuffer()
          const r = await fetch('/fuhuobi/api/icon', {
            method: 'POST',
          ...
          if (r && r.ok) {
            ...
            setStatus({ text: '✅ 已应用自定义图标，桌面快捷方式已更新。', kind: 'ok' })
            await refresh()
          } else setStatus({ text: (r && r.error) || '图标上传失败', kind: 'err' })
        } catch (e) { setStatus({ text: String((e && e.message) || e), kind: 'err' }) }
        setBusy(false)
      }

      const doClearIcon = async () => {
        setBusy(true); setStatus({ text: '', kind: '' })
        try {
          const r = await fetch('/fuhuobi/api/icon', { method: 'DELETE' }).then((x) => x.json()).catch(() => null)
          if (r && r.ok) {
            setHasCustomIcon(false)
            setStatus({ text: '已清除自定义图标，恢复默认金币图标。', kind: 'ok' })
            await refresh()
          } else setStatus({ text: (r && r.error) || '清除失败', kind: 'err' })
        } catch (e) { setStatus({ text: String((e && e.message) || e), kind: 'err' }) }
        setBusy(false)
      }
      ...
      return React.createElement('div', { className: 'gdb-wrap' },
        ...
        // ── 桌面快捷方式区块：勾选 + 自定义图标 + 实际路径 ──
        React.createElement('div', { className: 'gdb-hint', style: { display: 'flex', flexDirection: 'column', gap: '8px', border: '1px solid var(--dsw-alias-border-l2)', borderRadius: '8px', padding: '10px' } },
          // ① 勾选栏：是否自动创建桌面快捷方式
          ...
          React.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' } },
            React.createElement('span', { style: { fontSize: '12px' } }, '自定义图标：'),
            React.createElement('button', { className: 'gdb-btn', disabled: busy, onClick: () => fileInputRef.current && fileInputRef.current.click() }, '选择图片'),
            hasCustomIcon
              ? React.createElement('button', { className: 'gdb-btn gdb-danger', disabled: busy, onClick: doClearIcon }, '恢复默认')
              : null,
            React.createElement('input', {
              ref: fileInputRef,
              type: 'file',
              accept: hasPythonPil ? 'image/*,.ico' : '.ico',
              style: { display: 'none' },
              onChange: (e) => { const f = e.target.files && e.target.files[0]; if (f) doUploadIcon(f); e.target.value = '' },
            }),
            hasCustomIcon
              ? React.createElement('span', { style: { fontSize: '11px', color: 'var(--dsw-alias-state-success-primary)' } }, '✓ 已自定义')
              : React.createElement('span', { style: { fontSize: '11px', color: 'var(--dsw-alias-label-tertiary)' } }, hasPythonPil ? '默认金币图标' : '默认金币图标（仅支持 .ico）'),
          ),
          // 无 Python 提示
          !hasPythonPil
            ? React.createElement('div', { style: { fontSize: '11px', color: 'var(--dsw-alias-state-warning-primary)', marginTop: '2px' } },
                '未检测到 Python 依赖，只能上传 .ico 格式图标。安装 Python（含 PIL）后可上传 PNG/JPG。')
            : null,
          // ③ 实际路径展示
          React.createElement('div', { style: { display: 'flex', flexDirection: 'column', gap: '4px' } },
            React.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: '6px' } },
              React.createElement('span', { style: { fontSize: '11px' } }, '实际文件位置：'),
              React.createElement('code', { style: { fontSize: '11px', wordBreak: 'break-all', color: 'var(--dsw-alias-label-tertiary)' } }, paths.coinCmd || '—'),
              paths.coinCmdExists
                ? React.createElement('span', { style: { color: 'var(--dsw-alias-state-success-primary)', fontSize: '11px' } }, '✓')
                : null,
            ),
            React.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: '6px' } },
              React.createElement('span', { style: { fontSize: '11px' } }, '桌面快捷方式：'),
              React.createElement('code', { style: { fontSize: '11px', wordBreak: 'break-all', color: 'var(--dsw-alias-label-tertiary)' } }, paths.desktopShortcut || '—'),
              paths.desktopShortcutExists
                ? React.createElement('span', { style: { color: 'var(--dsw-alias-state-success-primary)', fontSize: '11px' } }, '✓')
                : null,
            ),
          ),
        ),
        React.createElement('div', { className: 'gdb-hint' }, '说明：「环境」指 DSH 的一个独立运行版本（如 web、desktop 等）。「用此复活币复活」会把你选的环境恢复到快照时的配置（包括插件和依赖），恢复前会自动存一份备份，随时可以再改回。完成后重启 DSH 生效。'),
    ...
    // ── 手动存复活币按钮：注入到设置区，与「硬性重启」并排（尺寸/位置一致）──
    // 不修改任何其他插件的代码：独立 DOM 注入，复刻 niao-quick-open 的
    // nio-rst 按钮结构（绝对定位 28x28 圆形 + hover tooltip）。点一下对当前
    // 状态存一枚复活币。图标用小硬币。
    const GUARD_RST_TIP = '存复活币'
    const GUARD_RST_SVG = [
      // 小硬币图标（复活币）
    ...
    function makeGuardRestartSvg() {
      ...
      svg.innerHTML = GUARD_RST_SVG
      return svg
    }

    // 对其它 UI 插件(如 dsh-better-sidebar)的重渲染/热更新鲁棒：把「样式存活」与
    // 「按钮存活」解耦；几何关键声明内联 important(样式表缺失也坏不了布局)；
    // body+head MutationObserver(rAF 节流)+ 心跳 + reconcile 状态机。绝不动
    // [data-nio-rst] 及其样式(只读存在性取 right 偏移)。
    const GUARD_RST_VERSION = 'v1' // 模块版本令牌：热重放后据此判定旧实例残留

    function isGuardBox(el) {
      try { return !!el && el instanceof Element && el.getBoundingClientRect && getComputedStyle(el).display !== 'contents' } catch { return false }
    }
    // 解析真实有盒容器(不选 display:contents 无盒元素)。4 级降级，对未来 DOM 改名免疫。
    function resolveGuardBox() {
      const byClass = document.querySelector('[class*="settingsArea"]')
      if (isGuardBox(byClass)) return byClass
      const slot = document.querySelector('[data-slot="sidebar.settings"]')
      if (slot) {
        if (slot.parentElement && isGuardBox(slot.parentElement)) return slot.parentElement
        if (isGuardBox(slot)) return slot
      }
      const trigger = document.querySelector('button[aria-haspopup="dialog"]')
      if (trigger && trigger.parentElement && isGuardBox(trigger.parentElement)) return trigger.parentElement
      return null
    }
    // 样式存活(与按钮存在性解耦)：head 之外被任何插件清掉，都会经 observer/心跳补回。
    function ensureGuardStyle() {
      try {
        if (document.getElementById('fuhuobi-rst-style')) return
        const style = document.createElement('style')
        style.id = 'fuhuobi-rst-style'
        style.setAttribute('data-owner', 'dsh-fuhuobi')
        // 只承载美学层；几何关键(绝对定位/尺寸/right)全部内联 important 在按钮上。
        style.textContent = [
          '.fuhuobi-rst{color:color-mix(in srgb,var(--dsw-alias-label-secondary) 55%,transparent);background:transparent;cursor:pointer;transition:color .15s ease,background .15s ease}',
          '.fuhuobi-rst:hover{background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-state-success-primary)}',
          '.fuhuobi-rst{font-family:-apple-system,BlinkMacSystemFont,\'Segoe UI\',Roboto,sans-serif}',
          '.fuhuobi-rst-busy{opacity:.6;cursor:wait}',
          '.fuhuobi-rst-tip{position:absolute;bottom:calc(100% + 8px);left:50%;transform:translateX(-50%);white-space:nowrap;background:var(--dsw-alias-tooltip-bg);color:var(--dsw-alias-label-primary);border:1px solid var(--dsw-alias-border-l2);border-radius:6px;padding:4px 8px;font-size:11px;line-height:15px;pointer-events:none;opacity:0;transition:opacity .12s ease;z-index:2147483001;font-family:-apple-system,BlinkMacSystemFont,\'Segoe UI\',Roboto,sans-serif}',
          '.fuhuobi-rst:hover .fuhuobi-rst-tip{opacity:1}',
        ].join('\n')
        document.head.appendChild(style)
      } catch { /* 美学失败不阻塞 */ }
    }
    function setGuardInline(btn, prop, value) {
      try { btn.style.setProperty(prop, value, 'important') } catch { /* ignore */ }
    }
    // 几何关键声明全部内联 important：永不依赖 head<style> 存活，样式表缺失也坏不了布局。
    function applyGuardGeometry(btn) {
      setGuardInline(btn, 'position', 'absolute')
      setGuardInline(btn, 'top', '50%')
      setGuardInline(btn, 'transform', 'translateY(-50%)')
      setGuardInline(btn, 'width', '28px')
      setGuardInline(btn, 'height', '28px')
      setGuardInline(btn, 'box-sizing', 'border-box')
      setGuardInline(btn, 'display', 'inline-flex')
      setGuardInline(btn, 'align-items', 'center')
      setGuardInline(btn, 'justify-content', 'center')
      setGuardInline(btn, 'border', 'none')
      setGuardInline(btn, 'border-radius', '50%')
      setGuardInline(btn, 'padding', '0')
      setGuardInline(btn, 'z-index', '10')
    }
    function buildGuardButton() {
      const btn = document.createElement('button')
      btn.type = 'button'
      btn.className = 'fuhuobi-rst'
      btn.setAttribute('data-fuhuobi-rst', '1')
      btn.dataset.fuhuobiVersion = GUARD_RST_VERSION
      btn.setAttribute('aria-label', GUARD_RST_TIP)
      btn.appendChild(makeGuardRestartSvg())
      const tip = document.createElement('span')
      tip.className = 'fuhuobi-rst-tip'
      tip.textContent = GUARD_RST_TIP
      btn.appendChild(tip)
      btn.addEventListener('click', (e) => {
        e.stopPropagation()
        const t0 = Date.now()
        btn.setAttribute('aria-label', '存币中…')
        btn.classList.add('fuhuobi-rst-busy')
        btn.disabled = true
        fetch('/fuhuobi/api/revive-coin', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({}),
        })
          .catch(() => {})
          .then(() => new Promise((resolve) => setTimeout(resolve, Math.max(0, 600 - (Date.now() - t0)))))
          .then(() => { btn.setAttribute('aria-label', GUARD_RST_TIP); btn.classList.remove('fuhuobi-rst-busy'); btn.disabled = false })
          .catch(() => { btn.setAttribute('aria-label', GUARD_RST_TIP); btn.classList.remove('fuhuobi-rst-busy'); btn.disabled = false })
      })
      applyGuardGeometry(btn)
      return btn
    }
    // reconcile 状态机：孤儿→领养移动；残留(STALE 令牌/多份)→清除重建；真消失→新建；
    // 在位错位→幂等重摆。全局恒 ≤1 实例，无自激，绝不影响其它插件。
    function reconcileGuardButton() {
      try {
        ensureGuardStyle()
        const box = resolveGuardBox()
        let btn = document.querySelector('[data-fuhuobi-rst]')
        const all = Array.from(document.querySelectorAll('[data-fuhuobi-rst]'))
        if (all.length > 1) {
          const keep = (box && all.find((b) => box.contains(b))) || all[0]
          for (const b of all) if (b !== keep) try { b.remove() } catch {}
          btn = keep
        }
        if (btn && btn.dataset.fuhuobiVersion !== GUARD_RST_VERSION) {
        ...
        if (!btn) {
          btn = buildGuardButton(); box.appendChild(btn)
        } else if (!btn.isConnected) {
          box.appendChild(btn)
        } else if (!box.contains(btn)) {
          box.appendChild(btn) // 领养移动：监听器随元素保留，不重建不重复
        }
        if (getComputedStyle(box).position === 'static') { try { box.style.position = 'relative' } catch {} }
        const nio = box.querySelector('[data-nio-rst]') // 只读存在性，绝不改它
        setGuardInline(btn, 'right', nio ? '42px' : '8px')
        const collapsed = box.closest('[class*="collapsed"]') !== null
        setGuardInline(btn, 'display', collapsed ? 'none' : 'inline-flex')
      } catch { /* 本按钮崩溃绝不致黑屏 */ }
    }
    // 启动 supervisor：body+head 双 observer(setTimeout 节流)+ 3s 心跳，替代定时轮询。
    // 用 setTimeout(0) 而非 requestAnimationFrame 节流——无头浏览器/后台标签页会节流
    // rAF，可能让按钮一直不出现；setTimeout(0) 在任何可见性下都触发。收敛后不再写
    // DOM，故不会自激。
    function startGuardSupervisor(ctx) {
      try {
        ensureGuardStyle()
        let running = false, disposed = false
        // 观测回调直接同步 reconcile(可重入标志防自激)，绝不经过定时器——
        // 无头/后台标签页会节流 setTimeout/setInterval，一旦依赖定时器，侧栏
        // 挂载后再也没机会补建按钮。同步 reconcile 收敛后不再写 DOM，不会自激。
        const schedule = () => {
          if (disposed || running) return
          running = true
          try { reconcileGuardButton() } finally { running = false }
        }
        ...
        const hb = setInterval(schedule, 3000)
        if (ctx && typeof ctx.effect === 'function') {
          // cordis ctx.effect 立即执行回调，并把回调的返回当作 disposer；
          // 必须返回一个清理函数，绝不能在这里直接 disconnect（否则注册瞬间
          // 就断开 observer，reconcile 再也跑不起来，按钮永远不会被建出来）。
          ctx.effect(() => () => {
            disposed = true
            clearTimeout(t); clearInterval(hb)
            try { moBody.disconnect() } catch {}
            try { moHead.disconnect() } catch {}
          }, 'fuhuobi: guarded-restart supervisor')
        }
      } catch { /* 静默 */ }
    }
    ...
    function apply(ctx) {
      ...
      // 0c. 守护重启按钮：注入到设置区，与硬性重启并排；对其它 UI 插件重渲染鲁棒。
      try {
        startGuardSupervisor(ctx)
      } catch { /* best effort */ }

      // 0d. Boot heartbeat (null-rendering occupant in an always-mounted slot).
      try {
